...
class TileBackground:
    ...
    def draw(self):
        ...
        # fill here
        tile_left = self.window_left // 800     # 타일의 너비
        tile_right = min((self.window_left + self.canvas_width) // 800 + 1, 3)
        # 3이 넘어가지 않도록 (인덱스 에러가 나지 않도록)
        left_offset = self.window_left % 800


        tile_bottom = self.window_bottom // 600     # ( 800 = 파일의 너비 )
        tile_top = min((self.window_bottom + self.canvas_height) // 600 + 1, 3)
        bottom_offset = self.window_bottom % 600    # ( 600 = 파일의 높이 )
